...
def callback(call):
    if call.data != 'else':
        ...
        bot.send_message(call.message.chat.id, f'Получается {round(res, 2)}. Можете заново вписать')
...
def my_currency(message):
    try:
        ...
        res = currency.convert(amount, values[0], values[1])
        bot.send_message(message.chat.id, f'Получается {round(res, 2)}. Можете заново вписать')
        bot.register_next_step_handler(message, summa)
    except Exception:
